...
@interface LPAMainNavC () <UIGestureRecognizerDelegate>

@end

@implementation LPAMainNavC

+ (void)initialize {
    
    if (self == [LPAMainNavC class]) {
        // 设置导航栏
        // 系统适配
        UINavigationBar *navBar = nil;
        if (CurrentSystemVersion < 9.0) {
            // 9.0 前支持的方法
            navBar = [UINavigationBar appearanceWhenContainedIn:[LPAMainNavC class], nil];
        } else {
            // 9.0 后支持的方法
            navBar = [UINavigationBar appearanceWhenContainedInInstancesOfClasses:@[[LPAMainNavC class]]];
        }
