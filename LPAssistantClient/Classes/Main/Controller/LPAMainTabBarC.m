...
#import "LPAMainTabBarC.h"

#import <BlocksKit.h>

#import "LPAMainNavC.h"
...
@implementation LPAMainTabBarC

+ (void)initialize {
    // 要加这个判断, 不然如果是子类第一次被使用也会调用这个方法
    if (self == [LPAMainTabBarC class]) {
...
- (UIViewController *)setUpOneViewController:(NSDictionary *)vcInfo {
    ...
    if (!clsName) {
        // 获取失败, 返回一个默认的控制器
        return [[NSClassFromString(@"LPAMainNavC") alloc] initWithRootViewController:[UIViewController new]];
    }
    ...
    return ({
        ...
        // 包装导航控制器, 并进行设置
        UINavigationController *nav = [[NSClassFromString(@"LPAMainNavC") alloc] initWithRootViewController:vc];
        //设置文字与图片的距离
